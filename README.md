...
│   └── customer/v1/
│       └── customer.proto       # CustomerService: 5 RPCs + WatchCustomerEvents stream
├── gen/                         # Code generated by protoc (committed — no protoc required)
│   └── customer/v1/
│       ├── customer.pb.go
│       └── customer_grpc.pb.go
...
When `GRPC_ENABLED=true`, a gRPC server starts on `GRPC_ADDR` (default `:9090`) alongside the Echo HTTP server. Both transports share the same `Service` and `QueryService` — no business logic is duplicated.
...
# Discover services (no proto file needed — reflection is enabled)
